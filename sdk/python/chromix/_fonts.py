...
def _write_fontconfig(fonts_dir: Path, template: str) -> str | None:
    try:
        ...
        config = template.replace("@FONTS_DIR@", escape(fonts_dir))
        config = config.replace("@CACHE_DIR@", escape(cache_dir))
        digest = hashlib.sha256(config.encode("utf-8")).hexdigest()
        config_path = Path(tempfile.gettempdir()) / f"chromix-fontconfig-{os.getuid()}-{digest}.conf"
        try:
